...
for (( i=1; i<=$2; i++  )); do echo "192.168.20.15$i network0$i" >> /etc/hosts; done
echo "192.168.20.51 manage01" >> /etc/hosts
...
# burrito all node disable NetworkManager  
#echo "GATEWAY=192.168.121.1" >> /etc/sysconfig/network-scripts/ifcfg-eth0
...
#sed -i "s/NM_CONTROLLED=yes/NM_CONTROLLED=no/" /etc/sysconfig/network-scripts/ifcfg-eth4
...
# burrito install package all node
sudo dnf -y install python3 epel-release


# burrito install package donwload control01
if [ `echo $(hostname)` = "control01" ]
then
sudo dnf -y install git python39 python3-cryptography
fi


# Ceph node Selinux enabled and Static IP eth0
for ((i=1; i<=3; i++))
do

if [ `echo $(hostname)` = "ceph0$i" ]
then
setenforce 1
