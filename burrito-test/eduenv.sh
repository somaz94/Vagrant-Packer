...
function _base_show_usage() {
  ...
  echo -e "  revert    [taco] or [cloudpc] Revert trainning VM server"
...
function _vm_stop_status() {
  ...
  virsh_status=$(virsh list --name | grep cloudpc)

  if [ -n "$virsh_status" ]; then
    echo "Some vm is running. It will stop all vm"
    for vm_name in `virsh list --name | grep cloudpc`; do virsh destroy $vm_name; done
    vagrant halt -f 
...
command=$1
arg=$2
running_vm_list=$(virsh list --name | grep cloudpc)
